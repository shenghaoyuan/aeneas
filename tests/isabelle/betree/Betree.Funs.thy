...
definition betree_load_leaf_node
  (id : u64) (st : state) :
  result (state × (betree_List_t (u64 × u64)))
  where
...
definition betree_NodeIdCounter_new : result betree_NodeIdCounter_t where
...
definition betree_NodeIdCounter_fresh_id
  (self : betree_NodeIdCounter_t) :
  result (u64 × betree_NodeIdCounter_t)
  where
  let i = u64_add next_node_id self 1 :: u64 in
  Ok (next_node_id self, (| next_node_id = i |))

(* [betree::betree::upsert_update]:
   Source: 'src/betree.rs', lines 234:0-273:1 *)
definition betree_upsert_update
  (prev : option u64) (st : betree_UpsertFunState_t) : result u64 where
  case prev of
  | None =>
    case st of
  ...
  | Some prev1 =>
    ...
    | Betree_UpsertFunState_Add v =>
      ...
      if u64_ge margin v then u64_add prev1 v else Ok core_num_U64_MAX
    | Betree_UpsertFunState_Sub v =>
      if u64_ge prev1 v then u64_sub prev1 v else Ok 0 :: u64

where
  
  (* [betree::betree::{betree::betree::List<T>}::len]: loop 0:
     Source: 'src/betree.rs', lines 279:8-282:9 *)
  fun betree_List_len_loop
    ('t) (self : betree_List_t 't) (len : u64) : result u64 where
    case self of
    | Betree_List_Cons _ tl =>
      let len1 = u64_add len 1 :: u64 in betree_List_len_loop tl len1
    | Betree_List_Nil => Ok len
  
  (* [betree::betree::{betree::betree::List<T>}::len]:
     Source: 'src/betree.rs', lines 276:4-284:5 *)
  definition betree_List_len ('t) (self : betree_List_t 't) : result u64 where
    betree_List_len_loop self 0 :: u64
  
  where
    
    (* [betree::betree::{betree::betree::List<T>}::reverse]: loop 0:
       Source: 'src/betree.rs', lines 306:8-310:9 *)
    fun betree_List_reverse_loop
      ('t) (self : betree_List_t 't) (out : betree_List_t 't) :
    ...
    definition betree_List_reverse
      ('t) (self : betree_List_t 't) : result (betree_List_t 't) where
    ...
    where
      ...
      fun betree_List_split_at_loop
        ('t) (n : u64) (beg : betree_List_t 't) (end1 : betree_List_t 't) :
      ...
      definition betree_List_pop_front
        ('t) (self : betree_List_t 't) :
        result ('t × (betree_List_t 't))
        where
      ...
      definition betree_List_hd
        ('t) (self : betree_List_t 't) : result 't where
        case self of
      ...
      definition betree_ListPairU64T_head_has_key
        ('t) (self : betree_List_t (u64 × 't)) (key : u64) : result bool where
        case self of
      ...
      where
        ...
        fun betree_ListPairU64T_partition_at_pivot_loop
          ('t) (pivot : u64) (beg : betree_List_t (u64 × 't))
          ...
          | Betree_List_Cons hd tl =>
            ...
            if u64_ge i pivot
            then
              betree_ListPairU64T_partition_at_pivot_loop pivot beg
                (Betree_List_Cons hd end1) tl
            else
              betree_ListPairU64T_partition_at_pivot_loop pivot
                (Betree_List_Cons hd beg) end1 tl
          | Betree_List_Nil =>
        ...
        definition betree_ListPairU64T_partition_at_pivot
          ('t) (self : betree_List_t (u64 × 't)) (pivot : u64) :
          result ((betree_List_t (u64 × 't)) × (betree_List_t (u64 × 't)))
          where
        ...
        definition betree_Leaf_split
          ...
          let (content0, content1) =
            betree_List_split_at content split_size params
          in
          let (pivot, _) = betree_List_hd content1 in
          let (id0, node_id_cnt1) = betree_NodeIdCounter_fresh_id node_id_cnt
          in
          let (id1, node_id_cnt2) = betree_NodeIdCounter_fresh_id node_id_cnt1
          in
          let (st1, _) = betree_store_leaf_node id0 content0 st in
          let (st2, _) = betree_store_leaf_node id1 content1 st1 in
          let n = Betree_Node_Leaf (| id = id0, size = split_size params |) in
          let n1 = Betree_Node_Leaf (| id = id1, size = split_size params |) in
          Ok (st2, ((| id = id self, pivot, left = n, right = n1 |),
            node_id_cnt2))
        ...
        where
          ...
          fun betree_Node_lookup_in_bindings_loop
            ...
            | Betree_List_Cons hd tl =>
              ...
              then Ok (Some i1)
                ...
                if u64_gt i key
                then Ok None
                else betree_Node_lookup_in_bindings_loop key tl
          ...
          where
            ...
            fun betree_Node_lookup_first_message_for_key_loop
              (key : u64) (msgs : betree_List_t (u64 × betree_Message_t)) :
              result ((betree_List_t (u64 × betree_Message_t)) ×
                (betree_List_t (u64 × betree_Message_t) ⇒ betree_List_t (u64
                × betree_Message_t)))
              where
              case msgs of
                ...
                then
                  ...
                  let back1 =
                    fun (ret : betree_List_t (u64 × betree_Message_t)) =>
                      let next_msgs1 = back ret in
                      Betree_List_Cons x next_msgs1
                  in
                  Ok (l, back1))
              | Betree_List_Nil =>
                Ok (Betree_List_Nil,
            ...
            where
              ...
              fun betree_Node_apply_upserts_loop
                (msgs : betree_List_t (u64 × betree_Message_t))
                (prev : option u64) (key : u64) :
                ...
                then (
                  ...
                  let msgs1 =
                    betree_List_push_front msgs (key, Betree_Message_Insert v)
                  in
                  Ok (v, msgs1))
              ...
              where
                ...
                fun betree_Internal_lookup_in_children
                  (self : betree_Internal_t) (key : u64) (st : state) :
                  result (state × ((option u64) × betree_Internal_t))
                  where
                  if u64_lt key pivot self
                  then (
                    let (st1, (o, n)) = betree_Node_lookup left self key st in
                    Ok (st1, (o, (| self with left := n |))))
                  else (
                    let (st1, (o, n)) = betree_Node_lookup right self key st in
                ...
                and betree_Node_lookup
                  ...
                  | Betree_Node_Internal node =>
                    let (st1, msgs) = betree_load_internal_node id node st in
                    ...
                    | Betree_List_Cons p _ =>
                      ...
                      then (
                        ...
                        | Betree_Message_Upsert _ =>
                          ...
                          in
                          let (v1, pending1) =
                            betree_Node_apply_upserts pending v key
                          in
                          let msgs1 =
                            lookup_first_message_for_key_back pending1
                          in
                          let (st3, _) =
                            betree_store_internal_node id node1 msgs1 st2
                          in
                  ...
                  | Betree_Node_Leaf node =>
                    let (st1, bindings) = betree_load_leaf_node id node st in
              ...
              where
                ...
                fun betree_Node_filter_messages_for_key_loop
                  (key : u64) (msgs : betree_List_t (u64 × betree_Message_t))
                  :
                  result (betree_List_t (u64 × betree_Message_t))
                  where
                ...
                where
                  ...
                  fun betree_Node_lookup_first_message_after_key_loop
                    (key : u64)
                    (msgs : betree_List_t (u64 × betree_Message_t)) :
                    result ((betree_List_t (u64 × betree_Message_t)) ×
                      (betree_List_t (u64 × betree_Message_t) ⇒
                      betree_List_t (u64 × betree_Message_t)))
                    where
                    case msgs of
                    | Betree_List_Cons p next_msgs =>
                      ...
                      then (
                        let (l, back) =
                          betree_Node_lookup_first_message_after_key_loop key
                            next_msgs
                        in
                        let back1 =
                          fun (ret : betree_List_t (u64 × betree_Message_t))
                  ...
                  definition betree_Node_apply_to_internal
                    (msgs : betree_List_t (u64 × betree_Message_t))
                    (key : u64) (new_msg : betree_Message_t) :
                    result (betree_List_t (u64 × betree_Message_t))
                    where
                    ...
                    then
                      ...
                      | Betree_Message_Insert _ =>
                        ...
                        in
                        let msgs3 = betree_List_push_front msgs2 (key, new_msg)
                        in
                      ...
                      | Betree_Message_Delete =>
                        ...
                        in
                        let msgs3 =
                          betree_List_push_front msgs2 (key,
                            Betree_Message_Delete)
                        in
                        Ok (lookup_first_message_for_key_back msgs3)
                      | Betree_Message_Upsert s =>
                        let (_, m) = betree_List_hd msgs1 in
                        case m of
                        ...
                        | Betree_Message_Upsert _ =>
                          let (msgs2, lookup_first_message_after_key_back) =
                            betree_Node_lookup_first_message_after_key key
                              msgs1
                          in
                          let msgs3 =
                            betree_List_push_front msgs2 (key, new_msg)
                          in
                          let msgs4 = lookup_first_message_after_key_back msgs3
                          in
                          Ok (lookup_first_message_for_key_back msgs4)
                    else (
                      let msgs2 = betree_List_push_front msgs1 (key, new_msg)
                      in
                  ...
                  where
                    ...
                    fun betree_Node_apply_messages_to_internal_loop
                      (msgs : betree_List_t (u64 × betree_Message_t))
                      ...
                      | Betree_List_Cons new_msg new_msgs_tl =>
                        ...
                        betree_Node_apply_messages_to_internal_loop msgs1
                          new_msgs_tl
                    ...
                    where
                      ...
                      fun betree_Node_lookup_mut_in_bindings_loop
                        (key : u64) (bindings : betree_List_t (u64 × u64)) :
                        result ((betree_List_t (u64 × u64)) × (betree_List_t
                          (u64 × u64) ⇒ betree_List_t (u64 × u64)))
                        where
                        case bindings of
                        | Betree_List_Cons hd tl =>
                          let (i, _) = hd in
                          ...
                          then
                            Ok (bindings,
                              fun (ret : betree_List_t (u64 × u64)) => ret)
                          else (
                            let (l, back) =
                              betree_Node_lookup_mut_in_bindings_loop key tl
                            in
                            let back1 =
                      ...
                      definition betree_Node_apply_to_leaf
                        ...
                        in
                        let b = betree_ListPairU64T_head_has_key bindings1 key
                        in
                        if b
                        then (
                          let (hd, bindings2) = betree_List_pop_front bindings1
                          in
                          case new_msg of
                          | Betree_Message_Insert v =>
                            let bindings3 =
                              betree_List_push_front bindings2 (key, v)
                            in
                          ...
                          | Betree_Message_Insert v =>
                            let bindings2 =
                              betree_List_push_front bindings1 (key, v)
                            in
                            Ok (lookup_mut_in_bindings_back bindings2)
                          | Betree_Message_Delete =>
                            Ok (lookup_mut_in_bindings_back bindings1)
                          | Betree_Message_Upsert s =>
                            let v = betree_upsert_update None s in
                            let bindings2 =
                              betree_List_push_front bindings1 (key, v)
                      ...
                      where
                        ...
                        fun betree_Node_apply_messages_to_leaf_loop
                          ...
                          | Betree_List_Cons new_msg new_msgs_tl =>
                            let (i, m) = new_msg in
                            let bindings1 =
                              betree_Node_apply_to_leaf bindings i m
                            in
                            betree_Node_apply_messages_to_leaf_loop bindings1
                              new_msgs_tl
                        ...
                        definition betree_Node_apply_messages_to_leaf
                          ...
                          result (betree_List_t (u64 × u64))
                          where
                          betree_Node_apply_messages_to_leaf_loop bindings
                            new_msgs
                        ...
                        where
                          ...
                          fun betree_Internal_flush
                            (self : betree_Internal_t)
                            (params : betree_Params_t)
                            (node_id_cnt : betree_NodeIdCounter_t)
                            (content : betree_List_t (u64 × betree_Message_t))
                            (st : state) :
                            result (state × ((betree_List_t (u64 ×
                              betree_Message_t)) × (betree_Internal_t ×
                              betree_NodeIdCounter_t)))
                            where
                            let (msgs_left, msgs_right) =
                              betree_ListPairU64T_partition_at_pivot content
                                pivot self
                            in
                            let len_left = betree_List_len msgs_left in
                            if u64_ge len_left min_flush_size params
                            then (
                              let (st1, (n, node_id_cnt1)) =
                                betree_Node_apply_messages left self params
                                  node_id_cnt msgs_left st
                              in
                              let len_right = betree_List_len msgs_right in
                              if u64_ge len_right min_flush_size params
                              then (
                                let (st2, (n1, node_id_cnt2)) =
                                  betree_Node_apply_messages right self params
                                    node_id_cnt1 msgs_right st1
                                in
                                Ok (st2, (Betree_List_Nil,
                                  ((| self with left := n, right := n1 |),
                                  node_id_cnt2))))
                              else
                                Ok (st1, (msgs_right,
                                  ((| self with left := n |), node_id_cnt1))))
                            else (
                              let (st1, (n, node_id_cnt1)) =
                                betree_Node_apply_messages right self params
                                  node_id_cnt msgs_right st
                              in
                              Ok (st1, (msgs_left, ((| self with right := n |),
                                node_id_cnt1))))
                          ...
                          and betree_Node_apply_messages
                            ...
                            | Betree_Node_Internal node =>
                              let (st1, content) =
                                betree_load_internal_node id node st
                              in
                              let content1 =
                                betree_Node_apply_messages_to_internal content
                                  msgs
                              in
                              let num_msgs = betree_List_len content1 in
                              if u64_ge num_msgs min_flush_size params
                              then (
                                let (st2, (content2, p)) =
                                  betree_Internal_flush node params node_id_cnt
                                    content1 st1
                                in
                                let (node1, node_id_cnt1) = p in
                                let (st3, _) =
                                  betree_store_internal_node id node1 content2
                                    st2
                                in
                                Ok (st3, (Betree_Node_Internal node1,
                                  node_id_cnt1)))
                              else (
                                let (st2, _) =
                                  betree_store_internal_node id node content1
                                    st1
                                in
                                Ok (st2, (self, node_id_cnt)))
                            | Betree_Node_Leaf node =>
                              let (st1, content) =
                                betree_load_leaf_node id node st
                              in
                              let content1 =
                                betree_Node_apply_messages_to_leaf content msgs
                              in
                              let len = betree_List_len content1 in
                              let i = u64_mul 2 :: u64 split_size params in
                              if u64_ge len i
                              then (
                                let (st2, (new_node, node_id_cnt1)) =
                                  betree_Leaf_split node content1 params
                                    node_id_cnt st1
                                in
                                let (st3, _) =
                                  betree_store_leaf_node id node
                                    Betree_List_Nil st2
                                in
                                Ok (st3, (Betree_Node_Internal new_node,
                                  node_id_cnt1)))
                              else (
                                let (st2, _) =
                                  betree_store_leaf_node id node content1 st1
                                in
                        ...
                        definition betree_Node_apply
                          ...
                          result (state × (betree_Node_t ×
                            betree_NodeIdCounter_t))
                          where
                          betree_Node_apply_messages self params node_id_cnt
                            (Betree_List_Cons (key, new_msg) Betree_List_Nil)
                            st
                        
                        (* [betree::betree::{betree::betree::BeTree}::new]:
                           Source: 'src/betree.rs', lines 848:4-862:5 *)
                        definition betree_BeTree_new
                          (min_flush_size : u64) (split_size : u64)
                          (st : state) :
                          result (state × betree_BeTree_t)
                          where
                          let node_id_cnt = betree_NodeIdCounter_new in
                          let (id, node_id_cnt1) =
                            betree_NodeIdCounter_fresh_id node_id_cnt
                          in
                          let (st1, _) =
                            betree_store_leaf_node id Betree_List_Nil st
                          in
                          Ok (st1,
                            (|
                              ...
                              root =
                                (Betree_Node_Leaf (| id, size = 0 :: u64 |))
                            |))
                        
                        (* [betree::betree::{betree::betree::BeTree}::apply]:
                           Source: 'src/betree.rs', lines 867:4-870:5 *)
                        definition betree_BeTree_apply
                          (self : betree_BeTree_t) (key : u64)
                          (msg : betree_Message_t) (st : state) :
                          result (state × betree_BeTree_t)
                          where
                          let (st1, (n, nic)) =
                            betree_Node_apply root self params self
                              node_id_cnt self key msg st
                          in
                          Ok (st1,
                            (| self with node_id_cnt := nic, root := n |))
                        
                        (* [betree::betree::{betree::betree::BeTree}::insert]:
                           Source: 'src/betree.rs', lines 873:4-876:5 *)
                        definition betree_BeTree_insert
                          (self : betree_BeTree_t) (key : u64) (value1 : u64)
                          (st : state) :
                          result (state × betree_BeTree_t)
                          where
                          betree_BeTree_apply self key (Betree_Message_Insert
                            value1) st
                        ...
                        definition betree_BeTree_upsert
                          ...
                          result (state × betree_BeTree_t)
                          where
                          betree_BeTree_apply self key (Betree_Message_Upsert
                            upd) st
                        ...
                        definition betree_BeTree_lookup
                          ...
                          where
                          let (st1, (o, n)) =
                            betree_Node_lookup root self key st
                          in
                        ...
                        (* Unit test: main should evaluate to Ok () *)
