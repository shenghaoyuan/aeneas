...
def use_extend_from_slice
  (corecloneCloneInst : core.clone.Clone T) (v : alloc.vec.Vec T) (s : Slice T)
  :
...
def use_alloc_with_capacity (n : Usize) : Result (alloc.vec.Vec T) :=
  ok (alloc.vec.Vec.with_capacity T n)

/- [vec::from_elem]:
   Source: 'tests/src/vec.rs', lines 13:0-15:1 -/
def from_elem
  (corecloneCloneInst : core.clone.Clone T) (x : T) (n : Usize) :
  Result (alloc.vec.Vec T)
  :=
